...
int is_delimeter(char ch, char *delims);
...
/**
 * split_string_by_delimiters - takes a string and seperates its words
 *
...
/**
 * is_delimeter - checks if stream has delimitor char
 *
...
while (*(str + i))
{
if (is_delimeter(str[i], delims))
pending = 1;
else if (pending)
{
wLen++;
}
if (wLen > 0 && is_delimeter(str[i], delims))
break;
i++;
}
return (wLen);
